...
export default function Home() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setInput(""); // API 요청이 성공적으로 완료된 후에 입력창을 비움
    } catch (error) {
      console.error("오류:", error);
      // 오류 발생 시 입력값을 복원하지 않음
    } finally {
      setIsLoading(false);
            ...
            >
              {editingSessionId === session.id ? (
                ...
                <span
                  ...
                  className="flex-1 cursor-pointer truncate flex items-center text-gray-700"
                >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {currentSession?.messages.map((message, index) => (
            <AnimatedMessage
              ...
              isNew={index === currentSession.messages.length - 1}
            />
          ))}
          {isLoading && (
        ...
        >
          <div className="flex items-center space-x-2 max-w-4xl mx-auto">
            <input
              ...
              className="flex-1 rounded-full border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent pl-4 py-2 shadow-sm text-gray-800"
